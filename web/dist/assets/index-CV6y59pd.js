(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function e(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(r){if(r.ep)return;r.ep=!0;const n=e(r);fetch(r.href,n)}})();function c(t){const o=t.trim();if(o==="")return null;const e=Number(o);return Number.isFinite(e)?Math.trunc(e):null}function f(t){const o={};for(const e of t.skills){const s=e.name.trim(),r=e.value.trim();if(!s||r==="")continue;const n=Number(r);Number.isFinite(n)&&(o[s]=Math.trunc(n))}return{name:t.name.trim(),occupation:t.occupation.trim(),age:c(t.age),sex:t.sex.trim(),residence:t.residence.trim(),birthplace:t.birthplace.trim(),str:c(t.str),dex:c(t.dex),pow:c(t.pow),con:c(t.con),app:c(t.app),edu:c(t.edu),siz:c(t.siz),int:c(t.int),mov:c(t.mov),skills:o}}const y=[{key:"str",label:"근력"},{key:"dex",label:"민첩"},{key:"pow",label:"정신력"},{key:"con",label:"건강"},{key:"app",label:"외모"},{key:"edu",label:"교육"},{key:"siz",label:"크기"},{key:"int",label:"지능"},{key:"mov",label:"이동력"}],m=document.querySelector("#app");function b(){const t=window.location.pathname.split("/").filter(Boolean);return t[t.length-1]??""}function p(t){m.innerHTML="";const o=document.createElement("p");o.className="status status--error",o.textContent=t,m.appendChild(o)}async function h(){const t=b();m.innerHTML='<p class="status">확인 중...</p>';let o;try{o=await fetch(`/api/register/${t}`)}catch{p("네트워크 오류가 발생했습니다. 다시 시도해주세요.");return}if(!o.ok){p("링크가 만료되었거나 유효하지 않습니다. Discord에서 명령어를 다시 실행해주세요.");return}k(t)}function k(t){m.innerHTML=`
    <div class="card">
      <h1>캐릭터 등록</h1>
  ...
  `;const o=document.querySelector("#skill-rows"),e=document.querySelector("#form-error"),s=document.querySelector("#registration-form");function r(n="",i=""){const u=document.createElement("div");u.className="skill-row";const a=document.createElement("input");a.type="text",a.dataset.role="skill-name",a.placeholder="기능명",a.ariaLabel="기능명",a.value=n;const l=document.createElement("input");l.type="number",l.dataset.role="skill-value",l.placeholder="값",l.ariaLabel="값",l.value=i;const d=document.createElement("button");d.type="button",d.textContent="삭제",d.addEventListener("click",()=>u.remove()),u.append(a,l,d),o.appendChild(u)}document.querySelector("#add-skill-row").addEventListener("click",()=>r()),document.querySelector("#fill-common-skills").addEventListener("click",async()=>{const n=new Set(Array.from(o.querySelectorAll('[data-role="skill-name"]')).map(i=>i.value.trim()).filter(Boolean));try{const u=await(await fetch("/api/skills")).json();for(const a of u)n.has(a)||r(a)}catch{e.textContent="기능 목록을 불러오지 못했습니다.",e.hidden=!1}}),s.addEventListener("submit",async n=>{n.preventDefault(),e.hidden=!0;const i=w(s,o),u=f(i);let a;try{a=await fetch(`/api/register/${t}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch{e.textContent="네트워크 오류가 발생했습니다. 다시 시도해주세요.",e.hidden=!1;return}let l;try{l=await a.json()}catch{e.textContent="서버 응답을 처리할 수 없습니다. 다시 시도해주세요.",e.hidden=!1;return}if(!a.ok||!l.ok){e.textContent=l.error??"등록에 실패했습니다.",e.hidden=!1;return}m.innerHTML="";const d=document.createElement("p");d.className="status status--success",d.textContent=`${l.name} 캐릭터를 등록했습니다.`,m.appendChild(d)})}function w(t,o){const e=r=>{var n;return((n=t.elements.namedItem(r))==null?void 0:n.value)??""},s=Array.from(o.querySelectorAll(".skill-row")).map(r=>{var n,i;return{name:((n=r.querySelector('[data-role="skill-name"]'))==null?void 0:n.value)??"",value:((i=r.querySelector('[data-role="skill-value"]'))==null?void 0:i.value)??""}});return{name:e("name"),occupation:e("occupation"),age:e("age"),sex:e("sex"),residence:e("residence"),birthplace:e("birthplace"),str:e("str"),dex:e("dex"),pow:e("pow"),con:e("con"),app:e("app"),edu:e("edu"),siz:e("siz"),int:e("int"),mov:e("mov"),skills:s}}h();
