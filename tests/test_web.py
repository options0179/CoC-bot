...
def test_post_registration_accepts_weapon_list(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        upsert_mock = AsyncMock()
        monkeypatch.setattr("bot.web.upsert_character", upsert_mock)
        ...
        weapons = [{"name": "권총 .38", "damage": "1d10", "ammo": "6"}]
        app = create_app(pool=None)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/register/tok",
                json={"name": "탐사자", "pow": 50, "weapons": weapons},
            )
            assert resp.status == 200

        args, _ = upsert_mock.call_args
        assert args[3]["weapons"] == weapons

    asyncio.run(_body())


def test_post_registration_defaults_weapons_to_empty_list(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        upsert_mock = AsyncMock()
        monkeypatch.setattr("bot.web.upsert_character", upsert_mock)
        ...
        args, _ = upsert_mock.call_args
        assert args[3]["weapons"] == []

    asyncio.run(_body())


def test_post_registration_rejects_non_list_weapons(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        app = create_app(pool=None)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/register/tok",
                json={"name": "탐사자", "pow": 50, "weapons": {"name": "권총"}},
            )
            assert resp.status == 400
            body = await resp.json()
            assert "무기" in body["error"]

    asyncio.run(_body())


def test_post_registration_rejects_weapon_with_non_string_values(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        app = create_app(pool=None)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/register/tok",
                json={"name": "탐사자", "pow": 50, "weapons": [{"name": "권총", "ammo": 6}]},
            )
            assert resp.status == 400
            body = await resp.json()
            assert "무기" in body["error"]
...
def test_post_registration_rejects_weapon_without_name(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        app = create_app(pool=None)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/register/tok",
                json={"name": "탐사자", "pow": 50, "weapons": [{"damage": "1d10"}]},
            )
            assert resp.status == 400
            body = await resp.json()
            assert "무기" in body["error"]
...
def test_post_registration_accepts_bio_fields(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        upsert_mock = AsyncMock()
        monkeypatch.setattr("bot.web.upsert_character", upsert_mock)
        ...
        bio = {"gear": "트렌치코트, 손전등", "phobias": "어둠"}
        app = create_app(pool=None)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/register/tok", json={"name": "탐사자", "pow": 50, "bio": bio}
            )
            assert resp.status == 200

        args, _ = upsert_mock.call_args
        assert args[3]["bio"] == bio

    asyncio.run(_body())


def test_post_registration_defaults_bio_to_empty_dict(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        upsert_mock = AsyncMock()
        monkeypatch.setattr("bot.web.upsert_character", upsert_mock)
...
def test_post_registration_rejects_non_dict_bio(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        app = create_app(pool=None)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/register/tok", json={"name": "탐사자", "pow": 50, "bio": ["장비"]}
            )
            assert resp.status == 400
            body = await resp.json()
            assert "배경" in body["error"]

    asyncio.run(_body())


def test_post_registration_rejects_non_string_bio_value(monkeypatch):
    async def _body():
        _mock_valid_token(monkeypatch)
        app = create_app(pool=None)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/register/tok", json={"name": "탐사자", "pow": 50, "bio": {"gear": 3}}
            )
            assert resp.status == 400
            body = await resp.json()
            assert "배경" in body["error"]
