...
- `/산정 손실식` — 등록된 본인 캐릭터(PC)의 현재 SAN으로 SAN 체크를 굴리고 DB에 반영
  ...
  손실 5 이상이면 `temp_insanity`(일시적 광기), 직전 SAN의 1/5 이상이면 `indefinite_insanity`(부정형 광기)
  컬럼에도 실제로 기록한다(경고 표시는 그대로 유지, 한 번 켜진 플래그는 이후 굴림으로 꺼지지 않음)
- `/대립 내스킬 상대스킬` — 대립판정
...
- `/행동 설명` — 자유 서술 행동을 요약해 서술 임베드로 보여준다. 판정 필요 여부는 판단하지 않으며
  자동으로 판정을 굴리지 않는다 (판정은 `/판정`으로 직접 굴린다)

## 작동 원리

**상태 없는(stateless) 구조 + 선택적 데이터 저장.** `/판정`, `/대립`은 어디에도 데이터를
저장하지 않는다. 스킬값을 커맨드 인자로 받아 그 자리에서 계산하고 결과만 응답한다.
`/산정`은 예외로, 손실식만 인자로 받고 현재 SAN은 DB의 등록된 PC 캐릭터에서 자동으로
읽고 갱신한다(등록된 PC가 없으면 거부). 캐릭터시트 등록 기능(`/캐릭터등록`,
`/캐릭터조회`)은 사용자의 등록된 캐릭터를 Postgres에
저장하고 조회하며, 등록 시 HP/MP/SAN 최대치·시작치도 함께 계산해 저장한다.
`CoCBot.setup_hook()`이 cog를 로드하기 전에 DB 커넥션 풀부터
...
├── intent_analyzer.py             # 자유 서술 텍스트 → 행동 요약 (Gemini, 판정 필요 여부는 판단하지 않음)
├── bot/
...
│       ├── sanity.py            # /산정 커맨드
│       ├── opposed.py           # /대립 커맨드
│       ├── character.py         # /캐릭터등록, /캐릭터조회, /시나리오캐릭터등록 커맨드
...
| `intent_analyzer.py` | 플레이어의 자유 서술 텍스트를 Gemini로 요약해 `IntentResult`(행동 요약, 의도 유형, 목적, 대상)로 변환. 판정이 필요한지/어떤 스킬인지는 판단하지 않는다 |
...
| `bot/cogs/action.py` | `/행동` 슬래시 커맨드. 자유 서술을 `intent_analyzer.analyze_intent()`(`defer()` 후 `asyncio.to_thread`로 호출)로 요약해 서술 임베드로만 응답한다. 자동 판정 트리거는 없다 |
